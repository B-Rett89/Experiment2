...
class App(tk.Tk):
    ...
    def create_main_frame(self):
        ...
        ttk.Label(frame, text='New Post:').grid(row=2, column=0, sticky='w')
        self.post_var = tk.StringVar()
        ttk.Entry(frame, textvariable=self.post_var).grid(row=2, column=1, sticky='ew')
        ttk.Button(frame, text='Publish', command=self.publish_post).grid(row=2, column=2, padx=5)

        ttk.Label(frame, text='Your Posts:').grid(row=3, column=0, sticky='nw')
        self.post_box = tk.Text(frame, height=5, state='disabled')
        self.post_box.grid(row=3, column=1, columnspan=2, sticky='nsew')
        frame.rowconfigure(3, weight=1)

        ttk.Label(frame, text='Message:').grid(row=4, column=0, sticky='w')
        self.msg_var = tk.StringVar()
        ttk.Entry(frame, textvariable=self.msg_var).grid(row=4, column=1, sticky='ew')
        ttk.Button(frame, text='Send', command=self.send_message).grid(row=4, column=2, padx=5)

        ttk.Label(frame, text='Inbox:').grid(row=5, column=0, sticky='nw')
    ...
    def refresh_posts(self):
        try:
            ...
            for p in posts:
                self.post_box.insert('end', f"{p.timestamp}: {p.text}\n")
